...
function adminStyles(): string {
  return `
    ...
    * { box-sizing:border-box; }
    ...
    .change-banner { margin:0 0 20px; padding:14px 16px; border:1px solid #e6cf9d; border-radius:12px; background:#fff8e8; color:#6f4b12; font-size:13px; line-height:1.5; }
    .change-list { display:grid; gap:10px; margin-top:12px; }
    .change { padding:12px; border:1px solid var(--line); border-radius:10px; background:#fff; }
    .change strong { display:block; margin-bottom:7px; color:var(--ink); font-size:12px; }
    .change-values { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
    .change-value { min-width:0; padding:8px; border-radius:7px; background:#f5f7f6; color:#536159; font-size:11px; white-space:pre-wrap; overflow-wrap:anywhere; }
    .change-value.new { background:var(--brand-soft); color:var(--brand-dark); }
    ...
    .delete-published,.delete-rejected { width:100%; margin-top:18px; border:1px solid #efcaca; border-radius:10px; padding:11px 14px; background:var(--danger-soft); color:var(--danger); font-weight:850; }
    .delete-published:hover,.delete-rejected:hover { background:#f7dede; }
...
function adminScript(): string {
  return `
    ...
    const summary=(value)=>{if(Array.isArray(value))return value.map(item=>typeof item==='object'?(item.name||JSON.stringify(item)):item).join(', ');if(value&&typeof value==='object')return value.name||JSON.stringify(value);return value==null||value===''?'—':String(value);};
    const ratingText=(recipe)=>recipe&&Number(recipe.ratingCount)>0?(Number(recipe.averageRating||0).toFixed(1).replace(/\.0$/,'')+' ★ ('+recipe.ratingCount+' rating'+(recipe.ratingCount===1?'':'s')+')'):'No ratings yet';
    function recipeChanges(before,after){const fields=[['Name','name'],['Description','description'],['Instructions','instructions'],['Ingredients','ingredients'],['Method','method'],['Method name','methodName'],['Glassware','glasswareName'],['Garnish','garnish'],['Tags','tags'],['Servings','servings'],['Image','imageUrl'],['Video','video']];return fields.filter(([,key])=>comparable(before?.[key])!==comparable(after?.[key])).map(([label,key])=>({label,before:summary(before?.[key]),after:summary(after?.[key])}));}
    function changePanel(item){if(item.submissionType!=='update'||!item.publishedRecipe)return null;const changes=recipeChanges(item.publishedRecipe.recipe,item.recipe);const panel=node('div',{class:'change-banner'},[node('strong',{text:'Update to an existing recipe'}),document.createTextNode(changes.length?' Review the fields changed since the currently published version.':' The submitted recipe matches the currently published version.')]);if(changes.length){const list=node('div',{class:'change-list'});for(const change of changes)list.append(node('div',{class:'change'},[node('strong',{text:change.label}),node('div',{class:'change-values'},[node('div',{class:'change-value',text:'Published: '+change.before}),node('div',{class:'change-value new',text:'Submitted: '+change.after})])]));panel.append(list);}return panel;}
    ...
    function render(){grid.replaceChildren();if(state.loading&&!state.items.length){grid.append(node('div',{class:'loading',text:'Loading moderation queue…'}));return;}if(!state.items.length){grid.append(emptyState(state.status==='pending'?'Queue is clear':'Nothing here yet',state.status==='pending'?'There are no recipes waiting for review.':'No submissions have this status.'));return;}for(const item of state.items){const recipe=item.recipe||{};const media=imageBox(recipe,'card-image');media.append(node('span',{class:'badge',text:item.submissionType==='update'?'update':item.status}));const body=node('div',{class:'card-body'});body.append(node('h2',{text:recipe.name||'Untitled recipe'}),node('div',{class:'author',text:item.googleLogin||'Unknown author'}));const meta=node('div',{class:'meta'},[node('span',{text:(recipe.ingredients?.length||0)+' ingredients'}),node('span',{text:recipeMethod(recipe)}),node('span',{text:date(item.createdAt)}),item.publishedRecipe?node('span',{text:ratingText(item.publishedRecipe)}):null]);const review=node('button',{class:'review',type:'button',text:'Review recipe',onclick:()=>openSubmission(item.id)});body.append(meta,review);grid.append(node('article',{class:'card'},[media,body]));}}
    ...
    async function moderate(action){const notes=document.querySelector('#moderator-notes')?.value.trim()||undefined;const reason=document.querySelector('#rejection-reason')?.value.trim()||undefined;for(const button of document.querySelectorAll('.actions button'))button.disabled=true;try{await request('/api/admin/community/submissions/'+encodeURIComponent(state.selected.id),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,moderatorNotes:notes,rejectionReason:reason})});dialog.close();toast(action==='approve'?'Recipe approved and published.':'Recipe rejected.');await load();}catch(error){toast(error.message,true);for(const button of document.querySelectorAll('.actions button'))button.disabled=false;}}
    async function deletePublished(recipeId){if(!confirm('Delete this published recipe? It will disappear from the public feed and any pending updates will be rejected.'))return;const button=document.querySelector('.delete-published');if(button)button.disabled=true;try{await request('/api/admin/community/recipes/'+encodeURIComponent(recipeId),{method:'DELETE'});dialog.close();toast('Published recipe deleted.');await load();}catch(error){toast(error.message,true);if(button)button.disabled=false;}}
    async function deleteRejected(submissionId){if(!confirm('Permanently delete this rejected recipe? This removes its submission and moderation record and cannot be undone.'))return;const button=document.querySelector('.delete-rejected');if(button)button.disabled=true;try{await request('/api/admin/community/submissions/'+encodeURIComponent(submissionId),{method:'DELETE'});dialog.close();toast('Rejected recipe permanently deleted.');await load();}catch(error){toast(error.message,true);if(button)button.disabled=false;}}
