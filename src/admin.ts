...
function adminStyles(): string {
  return `
    ...
    * { box-sizing:border-box; }
    ...
    .hero { min-height:230px; margin-bottom:24px; border-radius:14px; overflow:hidden; background:linear-gradient(145deg,#dce9e0,#eef4f0); display:grid; place-items:center; color:#6f8778; font-size:48px; }
    ...
    .ingredient { padding:11px 0; border-bottom:1px solid #edf1ee; display:flex; justify-content:space-between; gap:18px; }
...
function adminScript(): string {
  return `
    ...
    function render(){grid.replaceChildren();if(state.loading&&!state.items.length){grid.append(node('div',{class:'loading',text:'Loading moderation queue…'}));return;}if(!state.items.length){const empty=node('div',{class:'empty'});empty.append(node('div',{},[node('strong',{text:state.status==='pending'?'Queue is clear':'Nothing here yet'}),node('span',{text:state.status==='pending'?'There are no recipes waiting for review.':'No submissions have this status.'})]));grid.append(empty);return;}for(const item of state.items){const recipe=item.recipe||{};const media=imageBox(recipe,'card-image');media.append(node('span',{class:'badge',text:item.status}));const body=node('div',{class:'card-body'});body.append(node('h2',{text:recipe.name||'Untitled recipe'}),node('div',{class:'author',text:item.googleLogin||'Unknown author'}));const meta=node('div',{class:'meta'},[node('span',{text:(recipe.ingredients?.length||0)+' ingredients'}),node('span',{text:recipeMethod(recipe)}),node('span',{text:date(item.createdAt)})]);const review=node('button',{class:'review',type:'button',text:'Review recipe',onclick:()=>openSubmission(item.id)});body.append(meta,review);grid.append(node('article',{class:'card'},[media,body]));}}
    async function request(url,options){const response=await fetch(url,{credentials:'same-origin',...options});const type=response.headers.get('content-type')||'';const body=type.includes('application/json')?await response.json():null;if(!response.ok)throw new Error(body?.error?.message||('Request failed ('+response.status+')'));return body;}
    async function load({append=false,cursor=null}={}){if(state.loading)return;state.loading=true;if(!append){state.items=[];state.nextCursor=null;}render();setStatus('Loading '+state.status+' submissions…');try{const suffix=cursor?'&cursor='+encodeURIComponent(cursor):'';const data=await request('/api/admin/community/submissions?status='+state.status+'&limit=24'+suffix);state.items=append?state.items.concat(data.items):data.items;state.nextCursor=data.nextCursor;setStatus(state.items.length+' submission'+(state.items.length===1?'':'s'));render();document.querySelector('#load-more').hidden=!state.nextCursor;updateCounts();}catch(error){setStatus(error.message,true);grid.replaceChildren(node('div',{class:'empty'},[node('div',{},[node('strong',{text:'Could not load submissions'}),node('span',{text:error.message})])]));}finally{state.loading=false;}}
    ...
    function renderDetail(item){const r=item.recipe||{};document.querySelector('#dialog-title').textContent=r.name||'Recipe review';const main=node('div',{class:'detail-main'});main.append(imageBox(r,'hero'),node('h2',{text:r.name||'Untitled recipe'}),node('p',{class:'description',text:r.description||'No description provided.'}));const ingredients=node('div',{class:'section'},[node('h3',{text:'Ingredients'})]);for(const ingredient of r.ingredients||[]){const info=node('div',{},[node('div',{class:'ingredient-name',text:ingredient.name}),ingredient.note||ingredient.description?node('small',{text:ingredient.note||ingredient.description}):null]);const amount=[ingredient.amount,ingredient.unitName||ingredient.unit].filter(v=>v!==undefined&&v!=='').join(' ');ingredients.append(node('div',{class:'ingredient'},[info,node('div',{class:'amount',text:amount||'—'})]));}main.append(ingredients);const steps=node('ol',{class:'instructions'});for(const step of instructionItems(r.instructions))steps.append(node('li',{text:step}));main.append(node('div',{class:'section'},[node('h3',{text:'Instructions'}),steps]));if(r.garnish)main.append(node('div',{class:'section'},[node('h3',{text:'Garnish'}),node('p',{class:'description',text:r.garnish})]));
...
<dialog id="review-dialog" aria-labelledby="dialog-title"><div class="dialog-head"><strong id="dialog-title">Recipe review</strong><button class="close" id="dialog-close" type="button" aria-label="Close">×</button></div><div id="dialog-content"></div></dialog><div class="toast" id="toast" role="status" aria-live="polite"></div><script nonce="${nonce}">${adminScript()}</script></body></html>`;
